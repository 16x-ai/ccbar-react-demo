...
给客户看的最小接入页：**React 19 + 坐席条 + 日志**。界面、日志与状态语义对齐 `D:\code\ccbar\index.html`（参考页），软电话用 **npm 上的 `@16x/webphone-sdk`**（`import { CCBarClient }`），不再用 `<script>` 加载全局对象。
...
所以日志面板里 `页面已就绪，等待签入` 会出现**两行** —— 这是 React 的开发期行为，不是 SDK 的问题
（生产构建只有一行；`new CCBarClient()` 与对未连接实例的 `dispose()` 都没有副作用，重复执行是安全的）。
...
`enableJsSipDebug()` 在挂载的 effect 里第一时间执行 —— JsSIP 是首次 `connect()` 时懒加载的，debug 包在模块初始化时读一次 `localStorage.debug`，**晚于那一刻设置就不生效**（这也是为什么它不能做成「保存后再生效」的设置项）。
...
| `npm test` | `node --test`，覆盖日志与状态文案、校验函数、Token 代理与会话接口、组件渲染冒烟 |
| `npm run typecheck` / `npm run build` | `tsc -b` / 生产构建（Node ≥ 22.18） |
...
> `tests/render.test.ts` 用 Vite 自己的 `ssrLoadModule` 把 `.tsx` 编译出来再 `renderToString`：
> node --test 的类型擦除不支持 JSX，测试里不能直接 import `.tsx`。
