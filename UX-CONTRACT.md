...
页面是 React 19 复刻的坐席条，结构与样式对齐 `D:/code/ccbar/index.html`（参考页）；软电话用 **npm 上的 `@16x/webphone-sdk`**（`CCBarClient` 核心 API，不用 SDK 自带 UI、不注册自定义元素、不需要引样式）。
...
- 拦截 console（`log/info/warn/error/debug`），命中 `JsSIP|WebSocket|Registration|registrar|sip:|UA[|transport|WebPhone` 的输出写进 `SIP` 面板（来源 `jssip`）。SIP 原文靠 `localStorage.debug='JsSIP:*'`（见 README「SIP 原文」）。
...
- **dev 下 `页面已就绪，等待签入` 会出现两行**：页面挂在 `StrictMode` 下，React 开发期会刻意把挂载跑两遍（挂载 → 卸载 → 再挂载）。生产构建只有一行。
...
平台在每次注册后的**第一次外呼**会回 480（`Q.850;cause=16`），新 SDK 把它包成 `call.failed` 的 `CCBarError`。页面在「注册后还没打通过任何一路」且错误内容命中 480 / 暂时不可用 / 超时 / 网络类时，按「首次失败」起算的 1.5s / 3s / 6s 三个时间点自动重拨（**额度按每次签入记账，一轮最多 3 次**：重拨自己再失败不会重置次数或重新计时，因此不会变成无限重拨）；某一路接通、退签、或额度用完即停止；已经有一路在响或在通话时跳过该时间点、后面的照走；用户手动拨号会取消当前链条。逻辑与计数在 `src/lib/callRetry.ts`（有单测），日志由 `usePhone` 翻译：`呼叫暂时不可用，1.5s / 3s / 6s 处自动重拨（本次签入最多 3 次）`、`自动重拨（第 N 次）<号码>`、`跳过第 N 次自动重拨：已有呼叫在响或在通话`、`自动重拨已用完，不再兜底（下次签入才会重新记账）`。

## 设置

API 主机、API KEY、API SECRET、内部分机、软电话 WSS（都必填；WSS 需 `wss://` 开头）与 SIP 注册有效期（默认 600 秒，交给服务端写进会话的 `sip.registerExpires`）。SIP 原文固定记录，不做成设置项（见 README「SIP 原文」）。KEY / SECRET 只出现在设置框和会话请求体里，不进日志；设置写入 `localStorage` 的 `ccbar.reactDemo.settings`。会话形态（默认旧平台 `token/fs` + `seat/account/get`，见 README）由构建变量 `VITE_LEGACY_PLATFORM` 决定，页面里不切换。无 alert/confirm。
